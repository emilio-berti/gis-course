...
plot(eu, "perimeter", type = "continuous", col = hcl.colors(length(eu), "Dark Mint"))
```
...
Buffering is achieved in _terra_ using `buffer(x, width)`.
...
In _terra_, neighbors of a polygon are obtained using `adjacent()`.
...
# then a convoluted code to show the number of neighbors per country
...
plot(eu, "neigh", col = hcl.colors(max(eu$neigh) + 1, "Dark Mint"))
```
