...
First, by specifying `byrow = TRUE`, we make sure that the values contained in `vals` are inserted into the matrix row-wise, i.e. $A_{1,1}$ = `vals[1]`, $A_{1,2}$ = `vals[2]`, etc.
...
```{r, echo=FALSE}
sp <- rast(matrix(sample(c(0, 1), 1e2, replace = TRUE, prob = c(10, 1)), 1e1, 1e1))
plot(sp)
```
...
for (i in seq_len(100)) {
  sp <- rast(matrix(sample(c(0, 1), 1e2, replace = TRUE, prob = c(10+i, 1)), 1e1, 1e1))
...
```{r}
S <- sum(s)
plot(S, col = hcl.colors(minmax(S)["max", ], "Zissou 1"))
```
